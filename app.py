...
st.set_page_config(page_title="OPERALAB - Avaliação de Resultados", layout="wide")
...
st.markdown(
    ...
    <div style="display:flex;align-items:center;gap:8px;">
      <h1 style="margin:0;font-size:26px;line-height:1.05;">OPERALAB&nbsp;&nbsp;-&nbsp;&nbsp;Avaliação de Resultados</h1>
    </div>
...
# ----------------------
# Pré‑avaliação por especificação
# ----------------------
...
with aba2:
    ...
    if df_in is None:
        ...
        if not spec_keys:
            st.warning("Nenhuma especificação disponível no catálogo.")
        else:
            ...
            # Evitar AttributeError: só usa startswith se spec_key for string
            is_conama357 = isinstance(spec_key, str) and (
                spec_key.startswith("CONAMA 357/2005") or spec_key == "CONAMA 357/2005 – Classes (selecionar)"
            )
            if is_conama357:
                ...
                st.info("Os limites numéricos dependem do perfil de classe/ambiente da 357/2005. Vou carregar os valores oficiais na próxima atualização.")
...
st.caption("© {} | OPERALAB - Avaliação de Resultados".format(pd.Timestamp.now().year))
